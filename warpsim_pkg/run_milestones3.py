...
from warpsim.validation import check_adm_constraints, check_adm_evolution_equation
from warpsim.adm_evolution import evolution_equation_residual
...
def main():
    ...
    print("\n[2b] Full ADM DYNAMICAL evolution equation for K_ij "
          "(adm_evolution.py) -- unlike the constraints above, this checks "
          "that the geometry genuinely EVOLVES the way Einstein's equations "
          "require between t-slices, not just that one slice is valid "
          "initial data. Two independent derivations of d(K_ij)/dt "
          "(full nonlinear RHS vs. autodiff time-derivative of the exact "
          "closed-form solution) compared at each point...")
    for p in test_points:
        res = evolution_equation_residual(p, params)
        print(f"    point {np.asarray(p)}: "
              f"max|residual|={float(jnp.max(jnp.abs(res))):.3e}")
    ev_report = check_adm_evolution_equation(params, test_points)
    print(f"    -> {'PASS' if ev_report['pass'] else 'FAIL'}  "
          f"(max_residual={ev_report['max_residual']:.2e})")
